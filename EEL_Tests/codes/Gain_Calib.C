...
void Gain_Calib::Loop() {
    ...
    Double_t *spe_ADC_top_vals = db.GetEntry_d("SPE_ADC_Top", frun);

    //Double_t *ADC2Energy_Top_vals = db.GetEntry_d("ADC2Energy_Top", frun);


    Double_t *must_have_bot_threshold = db.GetEntry_d("MUST_HAVE_ADC_SUM_Bot", frun);

    Double_t *peak_pos_max_bot_vals = db.GetEntry_d("PEAK_POS_MAX_Bot", frun);
    Double_t *peak_pos_min_bot_vals = db.GetEntry_d("PEAK_POS_MIN_Bot", frun);

    Double_t *spe_ADC_bot_vals = db.GetEntry_d("SPE_ADC_Bot", frun);
   ...
   // Double_t *ADC2Energy_Bot_vals = db.GetEntry_d("ADC2Energy_Bot", frun);
    ...
    //    cout << "  ===================================== " << endl;
    //    for (int i = 0; i < n_ch; i++) {
    //        cout << "ADC2Energy_Top_vals[" << i << "] = " << ADC2Energy_Top_vals[i] << endl;
    //    }
    ...
    TH1D * h_Edep_Top_[n_ch];

    TH1D * h_RAW_ADC_Bot_[n_ch];
    TH1D * h_Edep_Bot_[n_ch];
    ...
    for (int ii = 0; ii < n_PMT_ch; ii++) {
        ...
        h_Edep_Top_[ii] = new TH1D(Form("h_Edep_Top_%d", ii), "", 200, -200, 3500);

        h_RAW_ADC_Bot_[ii] = new TH1D(Form("h_RAW_ADC_Bot_%d", ii), "", 200, -200., 3500.);
        h_Edep_Bot_[ii] = new TH1D(Form("h_Edep_Bot_%d", ii), "", 200, -200, 3500);
    }
    ...
    for (Long64_t jentry = 0; jentry < nentries; jentry++) {
        ...
        bool bot_cosmic = (((peak_pos_3_[3] >= peak_pos_min && peak_pos_3_[3] < peak_pos_max) * signal_3_[3] > 200. &&
                (peak_pos_3_[2] >= peak_pos_min && peak_pos_3_[2] < peak_pos_max) * signal_3_[2] > 200. ||
                (peak_pos_3_[7] >= peak_pos_min && peak_pos_3_[7] < peak_pos_max) * signal_3_[7] > 200. &&
                (peak_pos_3_[6] >= peak_pos_min && peak_pos_3_[6] < peak_pos_max) * signal_3_[6] > 200.) &&
                ((peak_pos_3_[10] >= peak_pos_min && peak_pos_3_[10] < peak_pos_max) * signal_3_[10] > 200. &&
                (1) ||
                (peak_pos_3_[15] >= peak_pos_min && peak_pos_3_[15] < peak_pos_max) * signal_3_[15] > 200. &&
                (1)));
        ...
        if (top_cosmic) {
            ...
            for (int ch = 0; ch < n_ch; ch++) {
                ...
                if (!(peak_pos_2_[ch] > peak_pos_min_top_vals[ch] && peak_pos_2_[ch] < peak_pos_max_top_vals[ch])) {
                    continue;
                }

                // double Energy = signal_2_[ch] * ADC2Energy_Top_vals[ch];
                double Energy = 1;
        ...
        if (bot_cosmic) {
            ...
            for (int ch = 0; ch < n_ch; ch++) {
                ...
                if (!(peak_pos_1_[ch] > peak_pos_min_bot_vals[ch] && peak_pos_1_[ch] < peak_pos_max_bot_vals[ch])) {
                    continue;
                }

                //double Energy = signal_1_[ch] * ADC2Energy_Bot_vals[ch];
                double Energy = 1;
                ...
                for (int ind_must = 0; ind_must < ch_list_must_have[ch].size(); ind_must++) {
                    ...
                    must_have_ADC_sum = must_have_ADC_sum + signal_1_[cur_ch_must]*
                            (peak_pos_1_[cur_ch_must] > peak_pos_min_bot_vals[cur_ch_must] && peak_pos_1_[cur_ch_must] < peak_pos_max_bot_vals[cur_ch_must]);
                }

                bool must_have_flag = (must_have_ADC_sum > must_have_bot_threshold[ch]);

                bool must_not_have_flag = true;

                for (int ind_mustNOT = 0; ind_mustNOT < ch_list_mustNOT_have[ch].size(); ind_mustNOT++) {
                    int cur_ch_mustNOT = ch_list_mustNOT_have[ch].at(ind_mustNOT);
                    must_not_have_flag = must_not_have_flag * (signal_1_[cur_ch_mustNOT] < spe_ADC_bot_vals[cur_ch_mustNOT]);
                }

                //cout<<must_not_have_flag<<"    "<<

                if (must_have_flag && must_not_have_flag) {
                    h_RAW_ADC_Bot_[ch]->Fill(signal_1_[ch]);

                    h_Edep_Bot_[ch]->Fill(Energy);
                }
